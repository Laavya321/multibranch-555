...
pipeline {
    agent none
    environment {
        PROJECT = "WELCOME TO DEVOPS B31 BATCH - Jenkins Class"
    }
    stages {
            ...
            stages {
                stage('Perform Packer Build') {
                    ...
                    steps { //Assume Role in SreeMasterAccount AWS Account
                        withAWS(role:'Devops30-Jenkins-AssumeRole', roleAccount:'591622755975', duration: 900, roleSessionName: 'jenkins-session') {
                            sh 'pwd'
                            sh 'ls -al'
                ...
                stage('Validate Ansible Playbook & Dry Run') {
                    ...
                    steps {
                        ...
                        withCredentials([file(credentialsId: 'MyKey', variable: 'ansiblepvtkey')]) {
                        sh "sudo cp \$ansiblepvtkey $WORKSPACE"
                        sh "ls -al"
                        sh "sudo ansible-playbook -i invfile docker-swarm.yml -u ansibleadmin --private-key=MyKey.pem --check"
                        }
                ...
                //   agent { label 'DEV' }  
                //   steps {
                //      sh "IP=\$(cat invfile | grep  '[0-9]' | head -1)"
                //      sh "sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://\"$IP\":8080 || true"
                //     }
                // }
            ...
            stages {
                stage('Perform Packer Build') {
                    ...
                    steps { //Assume Role in SREEK8S AWS Account
                        withAWS(role:'Devops30-Jenkins-AssumeRole', roleAccount:'209949574592', duration: 900, roleSessionName: 'jenkins-session') {
